  ...
  if (!indexContent.includes(`import { tool as ${toolNameCamelCase} } from './${toolName}'`)) {
    ...
    if (categoryName) {
      ...
      if (categoryExists) {
        ...
        const categoryPattern = new RegExp(`(\\{\\s*name:\\s*'${categoryName}'[^}]*?components:\\s*\\[)([^\\]]*?)(\\][^}]*?\\})`, 's')
        ...
        console.log('Available categories: Development, Crypto, Converter, Web, Images and videos, Network, Math, Measurement, Text, Data')
        process.exit(1)
      }
    }
